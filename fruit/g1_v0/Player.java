...
public class Player extends fruit.sim.Player
{
    ...
    public boolean pass(int[] bowl, int bowlId, int round, boolean canPick, boolean musTake) {
        ...
        double inc = 1.02; // should be a function of the number of players
        double prob_sum = 0.0;
    ...
    /*    for (int i = 0; i < bowl.length; i++) {
        ...
        double uniform_fruitsize=(double)bowlsize/12;

        for(int i=0; i<bowl.length;i++){

            double temp=(round == FIRST ? r0_seen_fruit[i] : r1_seen_fruit[i]);
          
            if(temp> uniform_fruitsize*bowls_seen[round])

                fruit_probs[i]+=(temp-uniform_fruitsize*bowls_seen[round])/(bowlsize*bowls_seen[round]); 
           else if(temp< uniform_fruitsize*bowls_seen[round])

               fruit_probs[i]-=reciprocal*bowls_seen[round];
                if(fruit_probs[i]<0.0)
                    fruit_probs[i]=0.0;
            prob_sum += fruit_probs[i];
        }
        for (int i = 0; i < bowl.length; i++) {
